...
describe('Fare Transfer Basic Tests', () => {
  ...
  describe('Test InitLedger', () => {
    ...
    it('should return success on InitLedger', async () => {
      ...
      await fareTransfer.InitLedger(transactionContext);
      let ret = JSON.parse(
        (await chaincodeStub.getState('customer1')).toString()
      );

      expect(ret).to.eql(
        Object.assign(customer, { docType: 'customer', LastTxnId: 'fare1' })
      );
    });
  });
  ...
  describe('Test EnrollCustomer ', () => {
    it('should return error on EnrollCustomer ', async () => {
      ...
      let fareTransfer = new FareTransfer();
      try {
        await fareTransfer.EnrollCustomer(
          transactionContext,
          customer.ID,
          customer.FirstName,
          customer.LastName,
    ...
    it('should return success on EnrollCustomer ', async () => {
      ...
      let ret = JSON.parse(
        (await chaincodeStub.getState(customer.ID)).toString()
      );
  ...
  describe('Test UpdatePrimaryTransit', () => {
    it('should return error on UpdatePrimaryTransit', async () => {
      ...
      try {
        await fareTransfer.UpdatePrimaryTransit(
          transactionContext,
          'customer7',
          'YRT'
        );
    ...
    it('should return success on UpdatePrimaryTransit', async () => {
      ...
      expect(ret).to.eql(
        Object.assign(
          { docType: 'customer', LastTxnId: 'fare1', TransitId: 'TTC' },
          customer
        )
      );
    });
  ...
  describe('Test ChargeFare', () => {
    ...
    it('should return success on ChargeFare', async () => {
      ...
      const ret = await fareTransfer.ChargeFare(
        transactionContext,
        customer.ID,
        'MI'
      );
      ...
      const ret1 = await fareTransfer.ChargeFare(
        transactionContext,
        customer.ID,
        'YRT'
      );
